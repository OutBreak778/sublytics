  ...
  const handleSubmit = async () => {
    ...
    if (signIn.status === "complete") {
      ...
      try {
        ...
        console.error("Client Trust error:", JSON.stringify(err, null, 2));

        if (err.errors?.[0]?.code === "strategy_for_user_invalid") {
          alert(
            "Verification method not available. Please contact support or try again later.",
          );
        }
      }
    } else {
